...
function hearingMeter(value) {
  const level = Math.max(0, Math.min(3, Number(value) || 0));
  const segments = [0, 1, 2].map((segment) => `<i class="${segment < level ? "filled" : ""}"></i>`).join("");
  return `<div class="hearing-meter" aria-label="Ses seviyesi ${level}/3"><span>SES</span>${segments}</div>`;
}
...
function drawScene(canvas, now) {
  ...
  const figureActive = state.threat === "figure";
  const figureRoom = room.kind === "figure";
  ...
  if (room.kind === "archive") {
    ...
    ctx.strokeStyle = figureActive ? "rgba(255, 93, 108, .5)" : "rgba(170, 116, 170, .3)";
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(width * .12, height * .26);
    ctx.lineTo(width * .12, height * .72);
    ctx.moveTo(width * .88, height * .26);
    ctx.lineTo(width * .88, height * .72);
    ctx.stroke();
    for (let ring = 1; ring <= 3; ring += 1) {
      ctx.globalAlpha = figureActive ? .34 - ring * .07 : .14 - ring * .025;
      ctx.beginPath();
      ctx.arc(width * .5, height * .32, width * (.045 + ring * .025), Math.PI * 1.12, Math.PI * 1.88);
      ctx.stroke();
      ctx.beginPath();
      ctx.arc(width * .5, height * .32, width * (.045 + ring * .025), Math.PI * .12, Math.PI * .88);
      ctx.stroke();
    }
    ctx.globalAlpha = 1;
  }
  ...
  doorGradient.addColorStop(0, dark ? figureRoom ? "#241326" : "#171320" : room.kind === "elevator" ? "#245650" : "#243444");
  doorGradient.addColorStop(.5, dark ? figureRoom ? "#120912" : "#0b0b13" : "#121c29");
  doorGradient.addColorStop(1, dark ? figureRoom ? "#32162b" : "#1e1428" : "#2b3947");
  ...
  if (danger || chase) {
    ...
    ctx.fill();
    if (figureActive) {
      // No eyes: the special encounter locates the team through sound.
      ctx.strokeStyle = "rgba(255, 93, 108, .8)";
      ctx.lineWidth = 2;
      ctx.shadowColor = "rgba(255, 51, 77, .8)";
      ctx.shadowBlur = 13;
      for (let ring = 1; ring <= 3; ring += 1) {
        const radius = width * (.035 + ring * .026) + Math.sin(now * .012 + ring) * 3;
        ctx.beginPath();
        ctx.arc(shadowX, shadowY - height * .15, radius, Math.PI * 1.12, Math.PI * 1.88);
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(shadowX, shadowY - height * .15, radius, Math.PI * .12, Math.PI * .88);
        ctx.stroke();
      }
      ctx.shadowBlur = 0;
    } else {
...
function mapMarkup() {
  ...
  for (let number = start; number <= end; number += 1) {
    ...
    const milestone = !visited && (number === 50 || number === 100) && number <= state.targetDoors;
    const classes = `${playerHere ? "current" : visited ? "visited" : "unknown"} ${milestone ? "milestone" : ""}`;
    const label = visited || playerHere ? padDoor(number) : milestone ? `M${number}` : "??";
    output += `<span class="map-node ${classes}" title="${milestone ? "Kör nöbetçi karşılaşması" : ""}">${label}</span>`;
  ...
  app.innerHTML = `<section class="join-layout">
    ...
    <section class="panel join-panel">
      ...
      <form id="join-form" class="stack-form">
        ...
        <label>Kapı sayısı <span>(yalnızca yeni lobide)</span><select name="doors"><option value="20">20 — kısa vardiya</option><option value="30">30 — standart</option><option value="40">40 — uzun gece</option><option value="50">50 — kör nöbetçi</option><option value="100">100 — son nöbet</option></select></label>
...
function normalActions(room, me) {
  ...
  const primaryDoor = figureActive
    ? actionButton(me.hidden ? "leave_hide" : "hide", me.hidden ? "DOLAPTAN ÇIK" : "SESSİZCE SAKLAN", "", me.hidden ? "main-action safe" : "main-action stealth-action")
    : actionButton("open", openLabel, "", "main-action");
  const searchLabel = figureActive ? "ARA · RİSK" : "ARA";
  const listenLabel = figureActive ? "DİNLE · RİSK" : "DİNLE";
  let html = `<div class="action-row primary-actions">${actionButton("search", searchLabel, "", room.lootAvailable ? "attention" : "")}${actionButton("listen", listenLabel)}${primaryDoor}</div>`;
  const hideButton = figureActive ? "" : actionButton(me.hidden ? "leave_hide" : "hide", me.hidden ? "DOLAPTAN ÇIK" : "SAKLAN", "", me.hidden ? "safe" : "");
  html += `<div class="action-row">${hideButton}${actionButton("back", "GERİ")}${actionButton("rest", "DİNLEN")}</div>`;
  ...
  if (items.coin && state.threat === "figure") html += actionButton("use", "JETON AT", "coin", "decoy");
  html += `</div>`;
...
function renderGame() {
  ...
  const figureSignal = state.threat === "figure" ? hearingMeter(state.figureNoise) : "";
  const threat = state.threat !== "none" ? `<div class="threat-banner ${state.threat === "figure" ? "figure-threat" : ""}"><span>!</span><div><strong>${escapeHtml(state.threatText)}</strong><small>${state.threat === "figure" ? "Görmüyor; en küçük sesi duyuyor. Herkes sessizce saklanmalı." : `${state.threatTurns} hamle içinde herkes saklanmalı.`}</small>${figureSignal}</div></div>` : "";
  ...
  app.innerHTML = `<section class="game-layout">
    ...
    <main class="game-grid">
      <section class="scene-panel panel ${room.dark && !room.lit ? "unlit" : ""} ${room.figureRoom ? "figure-room" : ""} ${state.threat !== "none" ? "under-threat" : ""}">
        <div class="scene-heading"><div><div class="panel-kicker">ODA ${padDoor(room.number)}</div><h1>${escapeHtml(room.name)}</h1></div><div class="scene-heading-meta"><span class="room-kind">${escapeHtml(kindLabels[room.kind] || room.kind)}</span>${room.figureRoom ? '<span class="milestone-chip">HEARING EVENT</span>' : ""}</div></div>
        <div class="scene-viewport ${state.threat !== "none" || state.phase === "chase" ? "alert" : ""} ${room.dark && !room.lit ? "low-light" : ""}" aria-label="Birinci şahıs oda görünümü"><canvas id="scene-canvas" width="960" height="540"></canvas><div class="scene-crosshair" aria-hidden="true"></div><div class="scene-scanlines" aria-hidden="true"></div><div class="scene-hud"><span>${state.threat === "figure" ? "HEARING ALERT" : room.dark && !room.lit ? "LIGHT LOW" : "SIGNAL OK"}</span><span>${state.phase === "chase" ? "RUN" : state.threat === "figure" ? "SILENCE" : "LOOK"}</span></div></div>
        <div class="room-copy"><p>${escapeHtml(room.description)}</p>${room.dark && !room.lit ? '<small class="muted">Oda karanlık. Çakmak kullanmadan arama yapamazsın.</small>' : ""}${room.figureRoom && !room.figureCleared ? '<small class="figure-text">Kör nöbetçi görüşe değil sese tepki veriyor. Sessiz kalmak için hemen saklan.</small>' : room.figureRoom ? '<small class="muted">Nöbetçi sesini kaybetti; salon şimdilik sakin.</small>' : ""}${room.locked ? '<small class="warning-text">İlerideki kapı kilitli; ekip anahtar veya maymuncuk aramalı.</small>' : ""}${room.falseDoorSeen ? '<small class="warning-text">Bu kapının yankısı yok. Dikkatli olun.</small>' : ""}</div>
