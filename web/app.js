...
function renderGame() {
  ...
  const threat = state.threat !== "none" ? `<div class="threat-banner ${state.threat === "figure" ? "figure-threat" : ""}"><span>!</span><div><strong>${escapeHtml(state.threatText)}</strong><small>${state.threat === "figure" ? `Görmüyor; ses seviyesi ${state.figureNoise || 0}/3. Herkes sessizce saklanmalı.` : `${state.threatTurns} hamle içinde herkes saklanmalı.`}</small></div></div>` : "";
