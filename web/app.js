...
function drawScene(canvas, now) {
  ...
  const bob = chase ? Math.sin(now * 0.018) * 3 : Math.sin(now * 0.0017) * 0.7;
  ...
  app.innerHTML = `<section class="game-layout">
    ...
    <main class="game-grid">
      <section class="scene-panel panel ${room.dark && !room.lit ? "unlit" : ""} ${state.threat !== "none" ? "under-threat" : ""}">
        ...
        <div class="scene-viewport" aria-label="Birinci şahıs oda görünümü"><canvas id="scene-canvas" width="960" height="540"></canvas><div class="scene-hud"><span>${room.dark && !room.lit ? "LIGHT LOW" : "SIGNAL OK"}</span><span>${state.phase === "chase" ? "RUN" : "LOOK"}</span></div></div>
