...
def puzzle(t: float) -> float:
    first = 0.40 * sine(t, 392.0) * envelope(t, 0.28, 0.003, 0.16)
    second = 0.38 * sine(t, 523.25) * envelope(t - 0.12, 0.34, 0.003, 0.18)
    third = 0.34 * sine(t, 659.25) * envelope(t - 0.24, 0.52, 0.004, 0.24)
    click = 0.18 * sine(t, 1_760.0) * envelope(t - 0.38, 0.30, 0.002, 0.16)
    return first + second + third + click


def error(t: float) -> float:
    low = 0.38 * sweep(t, 220.0, 91.0, 0.42) * envelope(t, 0.54, 0.002, 0.18)
    buzz = 0.25 * noise(t) * envelope(t - 0.05, 0.42, 0.002, 0.18)
    return low + buzz


def ambience(t: float) -> float:
    wind = 0.20 * noise(t) * envelope(t, 1.22, 0.12, 0.42)
    hum = 0.22 * sine(t, 47.0) * envelope(t, 1.28, 0.12, 0.42)
    overtone = 0.09 * sine(t, 97.0 + 7.0 * sine(t, 0.8)) * envelope(t, 1.18, 0.18, 0.34)
    return wind + hum + overtone
...
CUES = {
    ...
    "puzzle.wav": (0.90, puzzle),
    "error.wav": (0.60, error),
    "ambience.wav": (1.30, ambience),
