...
public:
    Game(int targetDoors, std::uint64_t seed, bool audioEnabled, Difficulty difficulty)
        ...
        secondChaseDoor_ = std::clamp((targetDoors_ * 3) / 4, chaseDoor_ + 3, targetDoors_ - 1);
        buildRooms();
...
private:
    ...
    void hide() {
        ...
        if (threat_ != Threat::None) {
            int hideFailureChance = 15;
            if (difficulty_ == Difficulty::Relaxed) {
                hideFailureChance = 7;
            } else if (difficulty_ == Difficulty::Nightmare) {
                hideFailureChance = 25;
            }
    ...
    void saveGame() {
        ...
        for (int number = 1; number <= targetDoors_; ++number) {
            ...
            file << room.visited << ' ' << room.searched << ' ' << room.lit << ' '
                 ...
                 << room.puzzleSolved << ' ' << room.falseDoorSeen << '\n';
        }
    ...
    void loadGame() {
        ...
        std::string header;
        int version = 0;
        if (!(file >> header >> version) || header != "NIGHTSHIFT_SAVE" || version < 1 || version > 4) {
