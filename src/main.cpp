...
private:
    ...
    void render() const {
        ...
        std::cout << "\n" << terminal::paint("Komutlar: yardım | bak | ara | kapıyı aç | saklan | çanta | durum | çıkış", "2;37") << "\n";
    }
    ...
    std::string inventorySummary() const {
        ...
        const std::vector<Item> order = {Item::BrassKey, Item::Lockpick, Item::Bandage, Item::Tonic,
                                         Item::Lighter, Item::Adrenaline, Item::Coin};
    ...
    void showHelp() {
        ...
        say("`dinle`: kapının arkasındaki sesleri kontrol et. `kaydet` / `yükle`: oyunu sakla.");
        say("Kısa komutlar: help, look, search, open, hide, leave, inventory, status, use, quit.");
    }
    ...
    void enterRoomEvent(Room& room) {
        ...
        if (room.number == chaseDoor_ && !chaseStarted_) {
            startChase();
            return;
        }

        const int chance = room.dark ? 25 : 16;
    ...
    void useItem(const std::vector<std::string>& tokens) {
        ...
        case Item::BrassKey:
            ...
            break;
        case Item::Coin:
            say("Jetonları burada harcayacak bir makine yok. Şimdilik sakla.");
            break;
        }
    }
    ...
    void disturb(int amount, const std::string& reason) {
        player_.sanity = std::max(0, player_.sanity - amount);
        say(reason + " Akıl -" + std::to_string(amount) + ".");
    ...
    void saveGame() {
        ...
        file << "NIGHTSHIFT_SAVE 1\n";
        file << targetDoors_ << ' ' << seed_ << ' ' << currentRoom_ << ' '
             << player_.health << ' ' << player_.sanity << ' ' << turns_ << '\n';
    ...
    void loadGame() {
        ...
        std::string header;
        int version = 0;
        if (!(file >> header >> version) || header != "NIGHTSHIFT_SAVE" || version != 1) {
