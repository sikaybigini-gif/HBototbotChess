...
bool validRole(const std::string& role) {
    return role == "scout" || role == "medic" || role == "engineer" || role == "guardian";
}
...
public:
    explicit GameState(int defaultDoors)
        : defaultDoors_(std::clamp(defaultDoors, 12, 100)),
          idCounter_(1),
        ...
        if (code.empty()) {
            ...
            if (params.count("doors")) {
                if (const auto doors = parseInt(params.at("doors")); doors.has_value()) {
                    newLobby.targetDoors = std::clamp(*doors, 12, 100);
                }
            }
    ...
    std::string action(const std::map<std::string, std::string>& params) {
        ...
        if (lobby.phase == LobbyPhase::Waiting) {
            handleLobbyAction(lobby, playerFound->second, command, value);
            return stateJson(lobby, playerId);
        }
...
private:
    ...
    void buildRooms(Lobby& lobby) {
        ...
        for (int number = 1; number <= lobby.targetDoors; ++number) {
            ...
            if (number == 1) {
                ...
                const int firstChase = firstChaseDoor(lobby.targetDoors);
                const int secondChase = secondChaseDoor(lobby.targetDoors, firstChase);
    ...
    void handleLobbyAction(Lobby& lobby, PlayerNet& player, const std::string& command, const std::string& value) {
        if (command == "role") {
            const std::string role = lowerAscii(trim(value));
            if (!validRole(role)) {
                addLog(lobby, "Geçersiz rol. Keşifçi, sağlıkçı, mühendis veya muhafız seç.");
            } else {
                player.role = role;
                addLog(lobby, player.name + " rolünü " + roleLabel(role) + " yaptı.");
            }
            return;
        }
        if (command == "ready") {
    ...
    void enterRoom(Lobby& lobby) {
        ...
        if (room.figureRoom && !room.figureCleared) {
            ...
            addLog(lobby, room.number == 50
                   ...
                   "danger.wav");
            addLog(lobby, "Sessizce saklanın. Kapıya uzanmak veya oyalanmak nöbetçinin dikkatini çeker.");
            return;
        }
    ...
    void tickThreat(Lobby& lobby, Threat expected, const PlayerNet* source = nullptr) {
        ...
        if (lobby.threatTurns > 0) {
            if (expected == Threat::Figure) {
                addLog(lobby, "Kör nöbetçi yaklaşan sesi dinliyor; " + std::to_string(lobby.threatTurns) + " hamle kaldı.");
            } else {
        ...
        for (auto& [id, player] : lobby.players) {
            if (player.alive && !player.hidden) {
                const int baseDamage = expected == Threat::Rattle ? 28 : expected == Threat::Figure ? 48 : 15;
                const int actualDamage = player.role == "guardian" ? std::max(1, baseDamage * 2 / 3) : baseDamage;
                hurt(lobby, player, actualDamage,
                     expected == Threat::Rattle ? "Gürültünün çarpması"
                                                : expected == Threat::Figure ? "Kör nöbetçinin duyduğu ses"
                                                                              : "Fısıltının gölgesi");
            }
        }
        if (expected == Threat::Figure) {
            lobby.rooms[static_cast<std::size_t>(lobby.currentDoor)].figureCleared = true;
            addLog(lobby, "Kör nöbetçi sesin izini kaybedip karanlığa çekildi.");
        }
    ...
    void handleExploreAction(Lobby& lobby, PlayerNet& player, const std::string& command, const std::string& rawValue) {
        ...
        if (consumesTime && threatBefore != Threat::None && lobby.threat == threatBefore && lobby.phase == LobbyPhase::Explore) {
            tickThreat(lobby, threatBefore, &player);
        }
    }
    ...
    void revivePlayer(Lobby& lobby, PlayerNet& actor, const std::string& targetId) {
        ...
        target->alive = true;
        target->hidden = false;
        target->health = actor.role == "medic" ? 55 : 35;
        target->sanity = actor.role == "medic" ? 55 : 40;
    ...
    void openDoor(Lobby& lobby, PlayerNet& player) {
        ...
        if (lobby.threat == Threat::Figure) {
            addLog(lobby, "Kör nöbetçi hâlâ burada. Kapı koluna uzanırsan çıkardığın sesi duyar; önce saklan.");
            return;
        }
        if (room.number == lobby.targetDoors) {
        ...
        if (room.falseDoor && !room.falseDoorSeen) {
            ...
            if (player.role == "scout") {
                addLog(lobby, player.name + " kapının yankısız olduğunu fark etti.");
            } else {
    ...
    void hidePlayer(Lobby& lobby, PlayerNet& player) {
        ...
        if (!room.hasCloset) {
            ...
            if (lobby.threat != Threat::None) {
                tickThreat(lobby, lobby.threat, &player);
            }
            return;
        }
        player.hidden = true;
        ...
        if (lobby.threat != Threat::None && allAliveHidden(lobby)) {
            const bool wasFigure = lobby.threat == Threat::Figure;
            lobby.threat = Threat::None;
            lobby.threatTurns = 0;
            lobby.figureNoise = 0;
            lobby.figureQuietActions = 0;
    ...
    void useItem(Lobby& lobby, PlayerNet& player, const std::string& item) {
        ...
        if (item == "lockpick") {
            ...
            } else if (itemCount(player, "lockpick") == 0) {
                addLog(lobby, "Çantanda maymuncuk yok.");
            } else {
                if (player.role != "engineer") {
                    takeItem(player, "lockpick");
                }
                room.locked = false;
                addLog(lobby, player.name + " maymuncukla kilidi açtı." +
                                  (player.role == "engineer" ? " Mühendis becerisi maymuncuğu korudu." : ""),
                       "key.wav");
            }
            return;
        }
        if (item == "coin") {
            if (lobby.threat != Threat::Figure) {
                addLog(lobby, "Jetonları servis arabasında harcayabilirsin.");
            } else if (!takeItem(player, "coin")) {
    ...
    void rest(Lobby& lobby, PlayerNet& player) {
        ...
        player.health = std::min(100, player.health + (player.role == "medic" ? 18 : 12));
        player.sanity = std::min(100, player.sanity + 8);
        if (player.role == "medic") {
            for (auto& [id, teammate] : lobby.players) {
                if (teammate.alive && teammate.id != player.id) {
                    teammate.health = std::min(100, teammate.health + 6);
                }
            }
        }
    ...
    void goBack(Lobby& lobby, PlayerNet& player) {
        if (lobby.threat == Threat::Figure) {
            addLog(lobby, "Kör nöbetçi geri dönüş sesini de duydu. Sessizce saklanın.");
            return;
        }
    ...
    std::string stateJson(const Lobby& lobby, const std::string& me) const {
        ...
        output << ",\"figureNoise\":" << lobby.figureNoise;
        ...
        output << ",\"figureRoom\":" << (room.figureRoom ? "true" : "false");
        output << ",\"figureCleared\":" << (room.figureCleared ? "true" : "false");
        ...
        for (const auto& [id, player] : lobby.players) {
            ...
            output << ",\"role\":\"" << jsonEscape(player.role) << "\"";
            output << ",\"roleText\":\"" << jsonEscape(roleLabel(player.role)) << "\"";
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (argument == "--help" || argument == "-h") {
            std::cout << "NÖBET co-op sunucusu\n\n"
                      ...
                      << "  --doors SAYI   Lobi haritası (12-100; 50 ve 100 özel nöbetçi odaları, varsayılan 20)\n";
            return 0;
        }
